...
function findExcelFiles(dir: string, files: string[] = []): string[] {
  if (!fs.existsSync(dir)) return files;
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const full = path.join(dir, entry.name);
    if (entry.isDirectory()) findExcelFiles(full, files);
    else if (/\.(xlsx|xls|xlsm|csv)$/i.test(entry.name)) files.push(full);
  }
  return files;
}
...
function resolveSafePath(filePath: string): string {
  ...
  const ALLOWED_ROOTS = [
    os.homedir(),
    "C:\\Users\\ferna\\Desktop\\UCB\\sis2\\DocumentosEmpresa",
  ];
  //const ALLOWED_ROOTS = [
  //DOCS_DIR,
  //os.homedir(),
...
async function buildExcelReport(params: {
  ...
  rows: ReportRow[];
  chart?: { type: "bar" | "line" | "pie"; label_key: string; series: ChartSeries[] };
  ...
  params.columns.forEach((col, i) => {
    ...
    const maxLen = Math.max(
      col.label.length,
      ...params.rows.map((r) => String(r[col.key] ?? "").length)
    );
...
async function buildWordReport(params: {
  ...
  for (const section of params.sections) {
    children.push(
      new Paragraph({ text: section.heading, heading: HeadingLevel.HEADING_1, spacing: { before: 360, after: 120 } })
    );
    children.push(
      new Paragraph({ children: [new TextRun({ text: section.content, size: 22 })], spacing: { after: 160 } })
    );
...
function buildHtmlDashboard(params: {
  title: string;
  kpis: { label: string; value: string | number; unit?: string; trend?: "up" | "down" | "neutral" }[];
  charts: {
    id: string;
    title: string;
...
function loadKpiConfig(): Record<string, unknown> {
...
function detectOutliers(rows: Record<string, unknown>[], key: string): number[] {
...
function readAllExcels(): {
  ...
  suministro: Record<string, unknown>[];
} {
  const files = findExcelFiles(DOCS_DIR);
  const result = { ventas: [], inventario: [], personal: [], proveedores: [], suministro: [] } as ReturnType<typeof readAllExcels>;

  for (const f of files) {
    const name = path.basename(f).toLowerCase();
    const wb = XLSX.readFile(f);
    const rows = sheetToRows(wb.Sheets[wb.SheetNames[0]]);
    if (name.includes("venta"))      result.ventas      = cleanRows(rows);
    if (name.includes("inventario")) result.inventario  = cleanRows(rows);
    if (name.includes("personal"))   result.personal    = cleanRows(rows);
    if (name.includes("proveedor"))  result.proveedores = cleanRows(rows);
    if (name.includes("suministro")) result.suministro  = cleanRows(rows);
  }
  return result;
}

async function buildFinancialReport(kpis: Record<string, unknown>): Promise<string> {
  ensureReportsDir();
  const wb = new ExcelJS.Workbook();
...
const server = new Server(
  { name: "excel-mcp-server", version: "2.0.0" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "list_excel_files") {
      const subdir = (args?.subdirectory as string) ?? "";
      const searchDir = subdir ? path.join(DOCS_DIR, subdir) : DOCS_DIR;
      if (!fs.existsSync(searchDir)) return text(`La carpeta no existe: ${searchDir}`);

      const files = findExcelFiles(searchDir);
      if (files.length === 0) return text(`No se encontraron archivos Excel en: ${searchDir}`);
    ...
    if (name === "read_excel_sheets") {
      const fp = resolveSafePath(args!.file_path as string);
      assertFileExists(fp);
    ...
    if (name === "read_excel_data") {
      ...
      assertFileExists(fp);
      const wb = XLSX.readFile(fp);
      const sheetName = (args?.sheet_name as string) ?? wb.SheetNames[0];
      if (!wb.SheetNames.includes(sheetName))
        return text(`Hoja "${sheetName}" no encontrada. Disponibles: ${wb.SheetNames.join(", ")}`);
      const maxRows = (args?.max_rows as number) ?? 200;
    ...
    if (name === "search_in_excel") {
      const fp = resolveSafePath(args!.file_path as string);
      assertFileExists(fp);
      const searchText = (args!.search_text as string).toLowerCase();
      const wb = XLSX.readFile(fp);
      const sheets = args?.sheet_name ? [args.sheet_name as string] : wb.SheetNames;
    ...
    if (name === "get_excel_summary") {
      ...
      if (rows.length === 0) return text("La hoja está vacía.");
    ...
    if (name === "generate_excel_report") {
      const outPath = await buildExcelReport({
        ...
        sheet_name: args!.sheet_name as string,
        columns:    args!.columns as ReportColumn[],
        rows:       args!.rows as ReportRow[],
        chart:      args?.chart as Parameters<typeof buildExcelReport>[0]["chart"],
        summary:    args?.summary as { label: string; value: string | number }[],
      });
    ...
    if (name === "generate_word_report") {
      const outPath = await buildWordReport({
        ...
        sections: args!.sections as Parameters<typeof buildWordReport>[0]["sections"],
        summary:  args?.summary as { label: string; value: string }[] | undefined,
      });
      return json({ ok: true, archivo_generado: outPath, mensaje: `Reporte guardado en ${outPath}` });
    }
    ...
    if (name === "generate_html_dashboard") {
      ...
      const html = buildHtmlDashboard({
        title:  args!.title as string,
        kpis:   args!.kpis as Parameters<typeof buildHtmlDashboard>[0]["kpis"],
        charts: args!.charts as Parameters<typeof buildHtmlDashboard>[0]["charts"],
        table:  args?.table as Parameters<typeof buildHtmlDashboard>[0]["table"],
      });
      const filename = `${sanitize(args!.title as string)}_${datestamp()}.html`;
      const outPath  = path.join(REPORTS_DIR, filename);
    ...
    if (name === "clean_data") {
      ...
      const outliers = args?.numeric_key
        ? detectOutliers(clean, args.numeric_key as string)
        : [];
      return json({
        ok: true,
  ...
  const rawPaths = args!.file_paths as string[];
  const resolvedPaths = rawPaths.map((p) => resolveSafePath(p));
  const missing = resolvedPaths.filter((p) => !fs.existsSync(p));
  if (missing.length > 0) return text(`Archivos no encontrados: ${missing.join(", ")}`);
